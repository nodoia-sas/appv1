...
export const VehicleCard = ({
  ...
  const categoryId =
    v.vehicleCategory?.id || v.vehicleCategoryId || v.vehicleCategoryId || "";
  const categoryName = (v.vehicleCategory?.name || v.vehicleCategoryName || "")
    .toString()
    .toLowerCase();
  let icon = "🚗";
  if (categoryName.includes("moto") || categoryId === MOTO_CATEGORY_ID)
    icon = "🏍️";

  const handleDelete = async () => {
    const ok = window.confirm(
      "¿Estás seguro de eliminar este vehículo? Esta acción es irreversible."
    );
    if (!ok) return;
    try {
      setDeletingVehicleId(deleteId);
      const res = await fetch(
        `/api/hooks/vehicles/delete?id=${encodeURIComponent(deleteId)}`,
        { method: "DELETE" }
      );
      if (!res.ok) {
        ...
        showMessage({
          type: "error",
          text: `No se pudo eliminar (${res.status})${txt ? " - " + txt : ""}`,
        });
        return;
      }
