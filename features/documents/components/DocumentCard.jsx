...
export const DocumentCard = ({
  ...
  const handleUpload = async () => {
    if (!file) {
      if (showMessage)
        showMessage({ type: "error", text: "Por favor selecciona un archivo" });
    ...
    try {
      ...
      if (!docId) {
        throw new Error(
          "No se encontró ID del documento. El documento debe existir previamente."
        );
      }
      ...
      if (showMessage)
        showMessage({
          type: "error",
          text: error.message || "Error al subir documento",
        });
    } finally {
      setLoading(false);
    }
  ...
  if (hasPath && !isEditing) {
    const isExpired = doc.expirationAt
      ? new Date(doc.expirationAt) < new Date()
      : false;
    ...
    return (
      <div className="text-sm">
        ...
        {doc.expirationAt && (
          <div className={`text-xs ${getExpiryColor(doc.expirationAt)}`}>
            {expiryLabel}{" "}
            {new Date(doc.expirationAt).toLocaleDateString("es-ES", {
              day: "numeric",
              month: "long",
              year: "numeric",
              timeZone: "UTC",
        ...
        <div className="flex items-center mt-2 justify-between">
          <a
            href={
              "https://transt-ia-filestore.s3.us-east-1.amazonaws.com/" +
              doc.path
            }
            target="_blank"
          ...
          >
            ...
            >
              <path
                ...
                d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M7.5 10.5l4.5 4.5m0 0l4.5-4.5m-4.5 4.5v-12"
              />
            </svg>
          </a>
    ...
    <div className="text-sm">
      ...
      {doc && !hasPath && (
        <div className="text-xs text-orange-600 mb-2">
          Documento registrado sin archivo
        </div>
      )}
