...
export const PersonalDocumentForm = ({ onSuccess, onCancel, showMessage }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!docType) {
      showMessage({ type: "error", text: "Selecciona el tipo de documento" });
      return;
    }
    if ((docType === "Licencia" || docType === "Pasaporte") && !expiryDate) {
      showMessage({ type: "error", text: "Ingresa la fecha de expiración" });
      return;
    }
    if (!file) {
      showMessage({ type: "error", text: "Adjunta un archivo (imagen o pdf)" });
      return;
    }
    ...
    try {
      ...
      showMessage({
        type: "error",
        text: err.message || "Error al guardar documento",
      });
    }
  };
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (v === "Cedula") setDocumentName("Cédula");
                  ...
                  setDocumentName("Licencia de Conducción");
              }}
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:bg-gray-50 transition-colors cursor-pointer relative">
              ...
              <div className="text-gray-500">
                {file ? (
                  ...
                  <span>
                    Arrastra un archivo o haz clic para seleccionar (PDF,
                    Imagen)
                  </span>
                )}
              </div>
