...
export const VehicleForm = ({ onSuccess, onCancel, showMessage }) => {
  ...
  const handleVehicleSubmit = async (e) => {
    ...
    if (vehicleLastTwo && !/^[0-9]{2}$/.test(vehicleLastTwo)) {
      showMessage({
        type: "error",
        text: "Los últimos 2 dígitos deben ser numéricos y tener 2 dígitos",
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showMessage({
          type: "error",
          text: `Error al guardar vehículo: ${res.status}${detail}`,
        });
        return;
      }
      ...
      showMessage(
        { type: "success", text: "Vehículo guardado correctamente." },
        4000
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setVehicleLastTwo(e.target.value.replace(/[^0-9]/g, ""))
              }
              className="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="Ej: 89"
            />
