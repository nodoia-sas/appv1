...
export const PersonalDocuments = ({ showMessage }) => {
  ...
  const fetchPersonalDocs = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showMessage({
          type: "error",
          text: `No fue posible cargar documentos personales (${res.status})`,
        });
        setPersonalDocs([]);
        return;
      }
      ...
      showMessage({
        type: "error",
        text: "Error al obtener documentos personales",
      });
      setPersonalDocs([]);
    } finally {
  ...
  const handleDelete = async (docId) => {
    ...
    try {
      const res = await fetch(`/api/hooks/documents/delete?id=${docId}`, {
        method: "DELETE",
      });
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Gestiona tus documentos de identidad y licencias
          </p>
        </div>
        <button
          ...
          {personalDocs && personalDocs.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
              <p className="text-gray-500">
                No tienes documentos personales registrados.
              </p>
              <button
                onClick={() => setShowPersonalForm(true)}
