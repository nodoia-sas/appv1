...
// Mock localStorage — functional in-memory storage
...
describe("PWA Functionality", () => {
  beforeEach(() => {
    ...
    // Reset navigator to default online state
    navigatorMock.onLine = true;
    delete navigatorMock.userAgent;
    ...
    // Provide ServiceWorkerRegistration so PWAUtils.getPWACapabilities() doesn't throw
    global.ServiceWorkerRegistration = { prototype: {} };

    // Reset matchMedia to return false by default (no standalone mode)
  ...
  describe("Integration Tests", () => {
    it("should handle offline to online transition", () => {
      // Spy on the real window.addEventListener before importing the module
      // (in jsdom, global.window === global, so windowMock doesn't intercept calls)
      ...
      // Simulate online event by finding and calling the registered handler
