...
import { getApiBase, getApiInfo } from './api-config'
...
export async function proxyRequest(req, res, {
    ...
    try {
        ...
        console.log(`[proxyRequest] Environment: ${apiInfo.name} (${apiInfo.environment})`)
        console.log(`[proxyRequest] API Base: ${apiInfo.finalUrl}`)
        if (apiInfo.isOverridden) {
            console.log(`[proxyRequest] Using API_URL override: ${apiInfo.finalUrl}`)
        }
